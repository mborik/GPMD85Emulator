/*	sigslot.h: Signal/Slot classes
	Copyright (c) 2002 Sarah Thompson <sarah@telergy.com>

	License: Public domain. You are free to use this code however you like,
	with the proviso that the author takes on no responsibility or liability
	for any use.

	Full documentation at http://sigslot.sourceforge.net/.
	...
	Threading modes:
		...
		- Behaviour in this mode is essentially the same as
		  multi_threaded_global, except that each signal, and each object
		  that inherits has_slots, all have their own mutex/critical section.
		  In practice, this means that mutex collisions (and hence context
		  switches) only happen if they are absolutely essential. However,
		  on some platforms, creating a lot of mutexes can slow down the
		  whole OS, so use this option with care.
*/
